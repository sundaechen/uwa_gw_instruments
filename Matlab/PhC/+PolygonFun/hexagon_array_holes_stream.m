...
format2 = strcat(num2str(dwell_time/0.1), format1, format1, ' \n');
...
for j = -2*nholes:2*nholes
    for i = -(2*nholes+mod(j,2)/2):(2*nholes+mod(j,2)/2)
        if i^2+(j*sin(pi/3))^2<=(structure_radius)^2/lattice_param^2
            % Create points
            [c, total_points] = PolygonFun.polygoncircles_stream(lattice_param_x*i, lattice_param_y*j, ...
...
%         % Write points into stream file
        fprintf(fid, format2, c/resolution);
        end
    end
end
