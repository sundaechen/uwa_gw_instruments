...
function square_array_holes(radius, lattice_param, structure_radius, nvertex, nholes, filename, show)
...
hold on
    ...
    for i = 1:nholes,
        % Create polygons
        c1 = PolygonFun.polygoncircles(position(1)+lattice_param*i, position(2), radius, nvertex);
        c2 = PolygonFun.polygoncircles(position(1)+lattice_param*i*0.5, position(2)+lattice_param*j*sqrt(3)/2, radius, nvertex);
