function hexagon_array_holes(offset_x, offset_y, radius, lattice_param, structure_radius, nvertex, nholes, filename, show)
...
for j = -2*nholes:2*nholes
    for i = -(2*nholes+mod(j,2)/2):(2*nholes+mod(j,2)/2)
        if i^2+(j*sin(pi/3))^2<=(structure_radius)^2/lattice_param^2
            % Create polygons
            c = PolygonFun.polygoncircles(lattice_param_x*i+offset_x, lattice_param_y*j+offset_y, radius, nvertex);
